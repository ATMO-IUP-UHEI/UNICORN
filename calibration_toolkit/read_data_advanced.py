...
def import_beacon_data(y=2025,m=5,d=19,h=0, readin=180, node=3, dir="/net/dsvr-02/mnt/data2/UNICORN/raw_data"):
    ...
    readin (int): number of files to read --> why 180?
    ...
    # die Zeiten in datetime values umwandeln
    ...
    # Take the column CO2. Resample it into 1-minute intervals ('1min'). 
    # Compute the mean CO₂ value for each 1-minute window. Store the result in a new column CO2_mean.
    ...
    # saves temperature and rh columns as floats
...
def import_CO_data(dir="/home/mdermend/reference_data"):
    try:
        df = pd.read_csv(f"{dir}/CO_ref_data_new.csv")
    except FileNotFoundError: #falls eine Datei fehlt
        print(f"Datei nicht gefunden")
...
def import_NO_data(dir="/home/mdermend/reference_data"):
    try:
        df = pd.read_csv(f"{dir}/NOX_ref_data_new.csv")
    except FileNotFoundError: #falls eine Datei fehlt
        print(f"Datei nicht gefunden")
...
ref=import_highprecision_data() # stores reference data into variable 'ref'
